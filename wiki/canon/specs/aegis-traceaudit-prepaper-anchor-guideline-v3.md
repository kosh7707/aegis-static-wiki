---
title: "AEGIS TraceAudit Pre-Paper Anchor Guideline v3"
...
decision_tags: ["prepaper-anchor", "traceaudit", "auditability", "sast-triage", "human-audit", "evidence-ledger", "freeze-before-results", "v2-review-incorporated", "critic-pass", "v3-latest"]
related_pages: ["wiki/canon/specs/aegis-traceaudit-prepaper-anchor-guideline.md", "wiki/canon/specs/aegis-traceaudit-prepaper-anchor-guideline-v1.md", "wiki/canon/specs/aegis-traceaudit-prepaper-anchor-guideline-v2.md", "wiki/canon/specs/aegis-traceaudit-benchmark-master.md", "wiki/canon/specs/aegis-traceaudit-paper-dossier.md", "wiki/canon/specs/paper-analysis-pipeline-design.md", "wiki/canon/api/paper-analysis-api.md"]
---

# AEGIS TraceAudit Pre-Paper Anchor Guideline v3

> Status: v3 final pre-paper anchor guideline; external review incorporated; Critic PASS.
> Version note: v3 is the Critic-approved latest version. v1 is the pre-review snapshot; v2 is the external-review incorporation draft.
...
```text
If this guideline conflicts with older experiment-protocol documents,
  this guideline wins for paper claims and benchmark design.

If this guideline conflicts with S4/S5 API contracts,
  the API contract wins for implementation semantics.

If this guideline conflicts with later experiment results,
  the guideline controls interpretation categories, not the numerical findings.
```
...
```text
If natural B4 machine-wrong cases < 10,
  wrong-verdict detection is descriptive in Audit120.
...
Success if either passes the pre-registered CI/margin rule with Holm-style
family correction or an explicitly justified hierarchical endpoint plan.
...
## 29. v2 review incorporation

This v2 incorporates the second 2026-05-18 external review. Changes applied before Critic re-review:

- top status changed from pending-Critic to v2 review-incorporated / Critic pending;
- added reviewer-replacement stance, conflict-resolution rule, and RQ5 pre-experiment eligibility distinction;
- clarified SAST finding vs internal claim candidate scoring;
- added manuscript style rule for paper-facing S3/S4/S5 role names;
- strengthened S4 `bounded_partial` reviewer-facing semantics and S4 diagnostic export handling;
- expanded artifact release tiers and source/license gates;
- added experiment-readiness gate for file-backed/live S4/S5 artifacts;
- clarified TriageQ cannot rescue failed core gates;
- added Trace50-to-Audit120 eligible warning pool gate;
- upgraded Audit120 primary target to 80 cases and made 60-case design exploratory if denominators fail;
- separated natural wrong-verdict denominator from injected FaultBench faults;
- fixed B0/B1/B2 evidence logging via `key_evidence_refs_or_spans` and hidden renderer mapping;
- added reviewer ethics, inclusion criteria, and mixed-effects fallback;
- decoupled oracle evidence refs from AEGIS evidence refs;
- added FaultBench minimum sizes, C0 handling, cost definitions, S5 gate nuance, practical margins, endpoint family rule, metric misuse risks, B3≈B4 result branch, SARIF/CodeQL background, result-slot additions, and pre-experiment gates.

## 30. Critic review incorporation

Previous Critic verdict before v2: initial `PASS_WITH_CHANGES`; follow-up after incorporation: `PASS`.

V2 Critic re-review verdict: `PASS`.
...
## 31. Final v3 decision

This v3 guideline is the Critic-approved single pre-paper anchor guideline. It freezes paper identity, operational benchmark design, reviewer protocol, S3/S4/S5 boundaries, result interpretation, and pre-experiment gates while leaving empirical claim strength result-dependent. The unversioned latest alias should match this file.
