...
decision_tags: ["structured-output", "analysis-agent", "contract", "http-status", "state-machine", "agent-v1.1", "wp-0a", "schema-delegation", "claim-diagnostics", "generation-controls", "tool-schema-validation", "input-boundary"]
...
Public `/v1/tasks.constraints`는 기존 request shape를 유지하면서 optional camelCase generation overrides를 additive로 허용한다: `enableThinking`, `temperature`, `topP`, `topK`, `minP`, `presencePenalty`, `repetitionPenalty`. Unknown/snake_case public keys는 API boundary에서 거절한다. `constraints.maxTokens` cap은 `32768`로 정렬되었고, `agent_llm_max_tokens` / `agent_max_completion_tokens` 기본값도 `32768`이다.
