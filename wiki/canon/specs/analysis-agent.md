...
| GET | `/v1/health` | 서비스 상태 + prompt/version + KB/LLM 상태 |
...
1. `/v1/health.activePromptVersions`
   ...
   - `generate-poc: v1`
2. legacy taskType은 Analysis Agent가 직접 처리하지 않고 `UNKNOWN_TASK_TYPE`로 거절한다.
3. `deep-analyze`와 `generate-poc`의 top-level request/response shape는 유지한다.
4. analysis 결과는 구조화 JSON + audit 정보를 유지한다.
