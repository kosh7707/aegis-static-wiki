...
service_tags: ["s3"]
decision_tags: ["structured-output", "analysis-agent", "contract", "http-status"]
...
Analysis Agent는 자동차 임베디드 소프트웨어에 대해 **결정론적 Phase 1 + 구조화된 Phase 2 + strict structured finalization**을 수행하는 증거 기반 보안 심층 분석 서비스다.
...
5. **non-JSON silent success 금지** — 반복 non-JSON 최종 출력은 completed fallback으로 숨기지 않고 strict finalizer를 거치며, finalizer 실패 시 실패 응답을 반환한다.
6. **HTTP/task outcome 정렬** — `/v1/tasks`는 동기 endpoint이므로 terminal task failure를 HTTP 200으로 숨기지 않는다.
...
7. `policyFlags`에 `structured_finalizer`가 있으면 최종 JSON은 strict finalizer repair path를 거쳤다는 의미다.
...
- 누락된 required top-level 필드(`caveats`, `usedEvidenceRefs`, `policyFlags` 등)나 malformed claim 구조는 안전 기본값으로 normalize하지 않고 strict schema repair를 1회 수행한다.
- repair 후에도 required field/type/claim shape가 invalid이면 `validation_failed / INVALID_SCHEMA`로 실패한다.
...
### 최근 검증 결과
- analysis-agent full suite: **349 passed**
