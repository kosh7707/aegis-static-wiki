...
decision_tags: ["structured-output", "analysis-agent", "contract", "http-status", "state-machine", "agent-v1.1", "wp-0a", "schema-delegation", "claim-diagnostics", "generation-controls", "tool-schema-validation", "input-boundary", "topk-alignment", "transitional-deprecation", "regression-gate"]
related_pages: ["wiki/canon/handoff/s3/readme.md", "wiki/canon/api/analysis-agent-api.md", "wiki/canon/specs/s3-claim-evidence-state-machine/claim-lifecycle.md"]
---
...
5. **thinking-on 기본값** — S3는 hotN/deep-analysis 품질 경로에서 thinking을 끄지 않는다. service-local `LlmCaller`, strict JSON finalizer, `eval_runner`는 S7의 `enable_thinking=true` 기본 계약에 맞춰 동작한다.
6. **non-JSON silent success 금지** — 반복 non-JSON 최종 출력은 strict finalizer/RecoveryTriage를 거치며, S3가 honest envelope를 조립할 수 있으면 completed negative/inconclusive outcome으로 반환한다. envelope 조립 자체가 불가능하거나 dependency/runtime이 unavailable이면 task failure다.
7. **HTTP/task outcome 정렬** — `/v1/tasks`는 동기 endpoint이므로 true terminal task failure를 HTTP 200으로 숨기지 않는다.
8. **public surface 안정성 우선** — `/v1/tasks`, `/v1/health`, `/v1/models`, `/v1/prompts` top-level 의미를 내부 리팩토링으로 깨지 않는다.
...
- Phase 2 tool availability의 authoritative gate는 Phase 1 완료 후, AgentLoop가 `ToolRegistry.get_all_schemas()` / `get_available_schemas()`로 LLM tool schema를 노출하기 전의 `ToolRegistry` filtering이다. `ToolRouter.execute`는 실행 디스패처일 뿐 tool advertisement authority가 아니다.
...
4. finalizer/repair call은 S7 strict JSON mode를 사용한다. `LlmCaller`는 tool-less 호출에서 `response_format={"type":"json_object"}`, `X-AEGIS-Strict-JSON` 헤더, `chat_template_kwargs.enable_thinking=true`를 사용한다. Strict JSON은 final content를 JSON object로 강제하지만 thinking/reasoning 자체를 끄지 않는다.
5. finalizer/repair output은 일반 결과처럼 `ResultAssembler`, schema validator, evidence validator를 통과해야 한다.
6. unsupported/hallucinated evidence ref는 fuzzy correction 또는 fallback injection으로 권한 부여하지 않는다. raw evidence validation에서 실패하면 RecoveryTriage가 `REFS_OR_GROUNDING_DEFICIENT`로 분류한다.
7. valid caller input + live dependencies에서 S3가 schema-valid honest envelope를 조립할 수 있으면 finalizer/repair 실패, malformed claims, unsupported evidence, missing grounding slots는 task failure가 아니라 completed result-level outcome(`inconclusive`, `no_accepted_claims`, `repair_exhausted`, `poc_rejected`, `poc_inconclusive`)으로 반환한다.
8. task failure/non-2xx는 invalid caller input, unavailable dependency/runtime, hard timeout/cancellation, unsafe/out-of-authority request, 또는 envelope assembly 자체가 불가능한 경우로 제한한다.
9. finalizer/repair가 성공하면 `policyFlags`에 `structured_finalizer`가 포함될 수 있으며, consumer는 이를 repair-path 신호로 취급한다.

이 설계는 반복 non-JSON, missing required fields, malformed claims, unsupported evidence를 silent normalization으로 숨기지 않는다. 대신 completed-vs-clean을 분리하여 안정적으로 소비 가능한 envelope와 honest quality diagnostics를 함께 반환한다.

### S7 caller-owned generation controls / P16/P17 hardening (2026-04-29)

S7 Gateway의 `/v1/chat` 및 `/v1/async-chat-requests`가 caller-owned generation tuple을 요구하므로, Analysis Agent의 S7 호출은 더 이상 gateway hidden default에 의존하지 않는다. Service-local `app/agent_runtime/llm/generation_policy.py`가 named preset을 소유하며 `LlmCaller` sync/async body는 항상 다음 snake_case fields를 포함한다: `max_tokens`, `temperature`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `repetition_penalty`, `chat_template_kwargs.enable_thinking`.

Public `/v1/tasks.constraints`는 기존 request shape를 유지하면서 optional camelCase generation overrides를 additive로 허용한다: `enableThinking`, `temperature`, `topP`, `topK`, `minP`, `presencePenalty`, `repetitionPenalty`. `topK`는 S7/vLLM escape hatch와 맞춰 `-1` 이상을 허용하며, named preset 기본값은 양수(`20` 또는 strict repair `1`)로 유지한다. Unknown/snake_case public keys는 API boundary에서 거절한다. `constraints.maxTokens` cap은 `32768`로 정렬되었고, `agent_llm_max_tokens` / `agent_max_completion_tokens` 기본값도 `32768`이다.

Runtime preset ownership:
- Phase 2 evidence acquisition: `THINKING_GENERAL` plus request overrides.
- Generate-PoC initial draft: `THINKING_CODING` plus request overrides.
- strict JSON finalizer/schema/quality repair: `STRICT_JSON_REPAIR` plus request overrides.
- `TimeoutDefaults` is service-local and mirrors the S7 timeout contract: chat/default 1800s, eval client read 600s, repair/strict JSON 600s, and tool execution 120s. `eval_runner`, LLM caller caps, task deadlines, and tool executor defaults consume these constants rather than scattering literals.
- P7 rationale is preserved in code comments at Generate-PoC draft/repair and structured finalizer call sites so future changes do not silently collapse task-specific presets.
- `eval_runner` also emits the full tuple.

P10/P16/P17 follow-up status:
- First evidence-acquisition LLM turn uses `tool_choice="required"` only while tools are available, no successful tool call has happened, and the loop is not in forced-report/finalizer/repair mode.
- Tool call arguments are validated against registered S3 tool schemas before implementation dispatch. Invalid arguments return `ToolResult(success=false, error="schema_violation", content=<JSON retry hint>)` and do not consume budget/execute tools.
- Tool results and Generate-PoC source snippets are wrapped at the LLM-facing untrusted-data boundary. Raw evidence/audit data remains intact outside prompt rendering.

### S7 async ownership / thinking-on 소비
S3는 tool 없는 LLM 호출에서 S7의 async ownership surface를 우선 사용한다. 2026-04-28 S7 WR 소비 후 S3-owned LLM caller는 기본적으로 `enable_thinking=true`를 명시하며, `/no_think` prompt suffix를 사용하지 않는다.
...
- LLM repair output이 같은 invalid shape를 반복하더라도 S3 scaffold shape를 보존한 뒤 재검증한다. repair 후에도 required field/type/claim shape가 invalid이면 valid-input/live-runtime 조건에서는 completed non-clean envelope로 분류한다. bounded quality-repair exhaustion은 `poc_inconclusive`/`repair_exhausted`이며, 즉시 unsafe/ref/grounding-deficient PoC는 `poc_rejected`다. invalid caller input이나 runtime/dependency unavailable일 때만 task failure로 반환한다.
- unsupported/hallucinated evidence ref는 fuzzy correction 또는 trusted fallback injection으로 보정하지 않는다. raw grounding validation에서 실패하면 completed `poc_rejected`/`REFS_OR_GROUNDING_DEFICIENT` outcome으로 분류한다.
- command-injection PoC에는 side-effect based detection, randomized canary, non-destructive marker, base64-decoded destructive payload detection, quote/subshell/IFS escape caveat를 deterministic quality guard로 보강하되, `supportingEvidenceRefs` 또는 `location`을 제조하지 않는다. quality repair는 configurable cap(`poc_quality_repair_max_attempts`, default 2)과 request-scoped budget guard 안에서만 수행된다.
- Quality repair가 cap/budget으로 exhausted 되면 completed envelope는 `qualityOutcome=repair_exhausted`, `pocOutcome=poc_inconclusive`를 사용한다. 즉시 unsafe/ref/grounding-deficient PoC는 계속 `poc_rejected`다.

---

## 8. 현재 검증 기준 (2026-04-28)
...
### 최근 검증 결과
- analysis-agent full suite: **558 passed in 5.58s**
- build-agent sibling suite for S3 boundary regression: **301 passed in 0.55s**
...
- 2026-05-03 S3 LLM readiness gate: **PASS** (`services/analysis-agent/tests/test_s3_llm_readiness_gate.py`, local `.omx/context/s3-llm-readiness-gate-20260503.py`), including P6 `eval/eval_runner.py` tuple coverage and P16 boundary-marker neutralization
...
- post-fix P1/P2 coverage closes KB-timeout, PoC repair timeout, partial Phase1→2 prompt/tool gating, multi-turn acquisition dedup, and SAST exactly-once gaps.
...
## 2026-04-27 WP-0a schema-delegation note

The public response-schema details for claim lifecycle state are intentionally delegated to [[wiki/canon/api/analysis-agent-api|Analysis Agent API 명세]]. The implementation rule for the paper-remediation workstream is:

- `result.claims[]` contains accepted final claims only.
- Non-accepted lifecycle states (`candidate`, `under_evidenced`, `rejected`, non-final `needs_human_review`) appear in bounded `result.claimDiagnostics[]` and audit/recovery diagnostics.
- Evidence acquisition failures and negative attempts are diagnostic inputs, not vulnerability proof and not task-level failures when S3 can assemble an honest completed envelope.
