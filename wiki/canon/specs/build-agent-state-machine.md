...
현재 `build-resolve` runtime에서 대표적으로 emit되는 public status:
...
`unsafe_output` / `empty_result`는 `TaskStatus` enum에 남아 있는 reserved 값이지만, 2026-05-06 기준 Build Agent `build-resolve`의 대표 runtime emission 경로로 취급하지 않는다.
...
| `script_planning` | loop/step/token budget is exhausted before a final build-domain envelope can be assembled | `task_failed` | `status="budget_exceeded"` |
| `script_planning` | LLM returns a schema-valid final envelope diagnosing synthesis/material deficiency before budget exhaustion | `completed_non_clean` | `status="completed"`, `cleanPass=false`, likely `missing_materials`, `compile_failed`, or `inconclusive` |
...
| any active state | S7 unavailable, hard timeout, cancellation, resource exhaustion, or unhandled internal exception prevents envelope assembly | `task_failed` | non-completed status such as `model_error`, `timeout`, or `budget_exceeded` |
...
| S7 output has empty tool calls / reasoning-only no action | retry if budget remains; strict-json/empty-output deficiencies can be assembled as completed non-clean; loop exhaustion maps to `budget_exceeded` |
