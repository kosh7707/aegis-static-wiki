...
**원칙**: 자유 텍스트 엔드포인트를 만들지 않는다. `/v1/tasks`는 taskType 기반으로 라우팅되며, `/v1/chat`은 기본적으로 LLM Engine의 OpenAI API를 투명하게 중계한다. 다만 2026-04-14부터 caller가 `X-AEGIS-Strict-JSON: true`를 보내면 Gateway가 opt-in strict JSON 모드를 적용해 `response_format=json_object`, JSON object 응답 검증/정규화, caller-facing `reasoning` scrub을 수행한다. `enable_thinking`은 caller-owned 필수 boolean이며 S7이 기본값을 주입하지 않는다.
...
- `GET /v1/health` — Circuit Breaker 상태 + additive `activeRequestCount` / `requestSummary` control signal
...
- request-aware health tracking: `/v1/tasks`, `/v1/chat`, `/v1/async-chat-requests` 모두 `requestId` 기반 in-flight summary를 노출하며, `llm-inference` 중에는 `localAckState="transport-only"`로 살아있지만 세부 progress-unproven 상태를 표현할 수 있다. async ownership에서는 이 상태가 elapsed-age-only abort 근거가 아니다
