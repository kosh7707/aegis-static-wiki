...
- caller가 `X-AEGIS-Strict-JSON: true`를 보낸 async request는 `/v1/chat` strict JSON과 동일하게 `response_format=json_object` 제어와 `enable_thinking=true` effective 기본값을 적용한다
...
- **Strict JSON / thinking**: `X-AEGIS-Strict-JSON: true`는 계속 지원한다. Gateway가 `response_format={"type":"json_object"}`와 effective default `chat_template_kwargs.enable_thinking=true`를 적용하고, 성공 응답의 `choices[0].message.content`를 parseable top-level JSON object로 검증/compact 정규화하며 `message.reasoning`은 `null`로 scrub한다. `<think>...</think>` 또는 일반 텍스트가 final JSON content에 섞이면 성공으로 반환하지 않고 `502 strict_json_contract_violation` 계열 실패로 처리한다. 따라서 S3 strict finalizer content는 JSON-only로 간주해도 된다.
...
- **Effective context / output limits**: live Engine `max_model_len=131072`; Gateway model registry `contextLimit=131072`. `/v1/tasks.constraints.maxTokens`는 1..8192(기본 2048). `/v1/chat`은 caller의 `max_tokens`를 Engine에 전달하되 Gateway가 별도 truncation하지 않는다. Gateway prompt character guard는 `AEGIS_LLM_MAX_INPUT_CHARS=800000`이며, 이는 토큰 한도 보장이 아니라 과대 입력 방어선이다. Long-context extension beyond 131072는 현재 켜지지 않았다. 초과 요청은 Gateway guard 또는 vLLM context/token validation error로 실패하며, S7이 임의로 잘라 성공처럼 반환하지 않는다.
...
S7의 기본 effective thinking policy는 **`enable_thinking=true`** 이다. 이 기본값은 `/v1/chat`, `/v1/async-chat-requests`, `/v1/tasks` real-client path에 모두 적용한다. caller가 `chat_template_kwargs.enable_thinking`을 생략하거나 boolean이 아닌 값으로 보내면 Gateway가 `true`를 주입한다. caller가 boolean `false`를 명시한 경우에만 mechanical/non-reasoning 요청으로 보존한다.

strict JSON은 예외적으로 thinking을 끄는 경로가 아니다. Gateway는 strict JSON에서 `response_format={"type":"json_object"}`와 final content JSON object 검증/정규화를 적용하되, effective thinking 기본값은 `true`로 유지한다. successful strict JSON response의 `message.reasoning`은 caller-facing response에서 `null`로 scrub하지만, exchange log에는 원본 response와 `effectiveThinking`이 남는다.
...
- `RealLlmClient`: task path에서 thinking 기본 활성(`enable_thinking=true`), JSON structured output
