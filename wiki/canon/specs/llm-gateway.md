---
title: "S7. LLM Gateway 기능 명세 (AEGIS)"
...
| 서비스 | 포트/위치 | 역할 |
|--------|-----------|------|
| **LLM Gateway** | :8000 | 레거시 5개 taskType + `/v1/chat` 프록시 + phase-2 async ownership surface (LLM 단일 관문) |
| **LLM Engine** | DGX Spark `10.126.37.19:8000` | `Qwen/Qwen3.6-27B`, vLLM 0.20.0 + MTP=1 서빙 |
...
POST /v1/async-chat-requests         # async ownership submit
GET  /v1/async-chat-requests/{id}    # async ownership status
GET  /v1/async-chat-requests/{id}/result # async ownership result fetch
DELETE /v1/async-chat-requests/{id}  # async ownership cancel
...
**원칙**: 자유 텍스트 엔드포인트를 만들지 않는다. `/v1/tasks`는 taskType 기반 finite synchronous TaskResponse-envelope surface이며, taskType 기반으로 라우팅된다. `/v1/tasks`에는 durable status/result/cancel endpoint가 없고 `/v1/health?requestId=...`는 active progress/control visibility 전용이다. `/v1/chat`은 기본적으로 LLM Engine의 OpenAI API를 투명하게 중계한다. 다만 2026-04-14부터 caller가 `X-AEGIS-Strict-JSON: true`를 보내면 Gateway가 opt-in strict JSON 모드를 적용해 `response_format=json_object`, JSON object 응답 검증/정규화, caller-facing `reasoning` scrub을 수행한다. `enable_thinking`은 caller-owned 필수 boolean이며 S7이 기본값을 주입하지 않는다.

### 5.2+ phase-2 async ownership surface

강한 no-result-loss / reconnect-safe wait-while-alive semantics는 `/v1/chat`를 stretch하지 않고 별도 async ownership surface에서 제공한다.

핵심 원칙:
- `/v1/chat` = finite synchronous compatibility surface
...
- caller가 `X-AEGIS-Strict-JSON: true`를 보낸 async request는 `/v1/chat` strict JSON과 동일하게 `response_format=json_object` 제어를 적용하되 caller-supplied `enable_thinking` 값을 보존한다
...
2. status polling by `requestId`
3. result fetch by `requestId`
4. optional cancel
5. explicit expiry

Status state allowlist:
- `queued`
- `running`
- `completed`
- `failed`
...
`accepted`는 submit response에만 사용하고, status endpoint의 long-lived state set에는 넣지 않는다.

Health-control v2 update (2026-05-08): async ownership is the production long-running LLM surface. S7 no longer applies a fixed elapsed backend read ceiling to async non-streaming inference while the backend attempt is live and no terminal failure exists. `queued`/`running` with `localAckState=phase-advancing|transport-only` and no `blockedReason` are continue-wait states. Active `expiresAt` is refreshed as an ownership lease hint; terminal result/error retention remains at least 15 minutes after completion/failure/cancel. `/v1/chat` remains the finite synchronous compatibility surface with `X-Timeout-Seconds` default/max 1800s.

### 5.2++ S3 system-stability boundary confirmation (2026-04-25)

S7 confirms S3's requested boundary: valid S3 input plus live S7 dependency plus model-output deficiency may be treated by S3 as an output-deficiency/recovery path when S3 can assemble an honest schema-valid envelope. True backend unavailability, circuit-open, hard timeout, cancellation, async retention expiry, or oversized/caller-invalid input remains dependency/runtime/caller-shaping failure.

Strict JSON failures are machine-readable: sync `/v1/chat` returns HTTP 502 with `error=Strict JSON contract violated`, `strictJson=true`, and `retryable=true`; async strict failures become terminal `failed` with `blockedReason=strict_json_contract_violation`, `errorDetail`, and `retryable=true`. Async ownership states are bounded to `queued|running|completed|failed|cancelled|expired`; terminal records retain results/errors for at least 15 minutes. Under health-control v2, S3 should not use a fixed bounded async poll deadline as the controlling long-running policy while S7 reports `queued`/`running` without ack-break or `blockedReason`; terminal decisions come from S7 state/result/cancel/expiry/ack-break signals.

Health readiness update (2026-05-08): `/v1/health.status` is process liveness only. LLM availability is exposed by `ready`, `llmReady`, `degraded`, `degradeReasons`, `blockedReason`, and `dependencyStatus`. DGX/vLLM unreachable returns `status="ok"` with `ready=false`, `llmReady=false`, `degraded=true`, `degradeReasons=["llm_backend_unreachable"]`, `blockedReason="backend_unreachable"`. Circuit breaker `open`/`half_open` also makes readiness false. RAG disabled/degraded does not block LLM readiness. 2026-05-11 performance update: backend `/health` probing is cached for a short bounded freshness window (`AEGIS_LLM_HEALTH_CACHE_TTL_SECONDS`, default 1.0s) so repeated orchestrator health polling does not hammer the DGX/OpenVPN proxy. Responses expose `llmBackend.cached` and `llmBackend.cacheTtlMs`; stale OK/unreachable states are not reused beyond the TTL.

Known observability gap: `/v1/health` exposes model profiles, backend readiness, circuit breaker, active request count, concurrency, and compact request summary, but not an explicit cost tier or queue-depth/saturation percentage.
...
| 필드 | 설명 |
|------|------|
| profileId | 프로필 식별자 (예: `Qwen/Qwen3.6-27B-default`) |
| provider | 제공자/서버 |
| modelName | 모델명 |
| build | 빌드/버전 |
...
5. **정책 노출**: `/v1/health.rag`는 `topK`, `minScore`, `policy`를 노출한다. 현재 정책명은 `task-pipeline-context-enrichment`이며, S7은 RAG 결과를 자동으로 caller evidence catalog에 등록하지 않는다.
6. **품질 경계**: `AEGIS_RAG_MIN_SCORE` 기본값은 `0.35`다. 이 값 미만의 hit는 prompt에 넣지 않는다. RAG hit는 confidence의 `ragCoverage`와 audit `ragHits`에만 영향을 주며, EvidenceRef grounding gate를 대체하지 않는다.
...
- `GET /v1/async-chat-requests/{requestId}/result` — final result fetch
- `DELETE /v1/async-chat-requests/{requestId}` — best-effort cancel
- `GET /v1/health` — process liveness + explicit LLM readiness/degraded dependency status + bounded-freshness backend probe metadata + additive `activeRequestCount` / `requestSummary` control signal
...
- request-aware health tracking: `/v1/tasks`, `/v1/chat`, `/v1/async-chat-requests` 모두 `requestId` 기반 in-flight summary를 노출하며, `llm-inference` 중에는 `localAckState="transport-only"`로 살아있지만 세부 progress-unproven 상태를 표현할 수 있다. async ownership에서는 이 상태가 elapsed-age-only abort 근거가 아니다. `/v1/tasks` health summary is not durable task result recovery.
- async ownership surface는 durable `requestId`, `traceRequestId`, `resultReady`, `expiresAt`를 가진다. active `expiresAt`은 queued/running 동안 refresh되는 ownership lease hint이며, terminal retention은 terminal 전이 후 최소 15분이다
...
- Generation observability: exchange logs와 Prometheus가 `temperature`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `repetition_penalty`, effective thinking을 기록한다. Prometheus metric names: `aegis_llm_temperature`, `aegis_llm_top_p`, `aegis_llm_top_k`, `aegis_llm_min_p`, `aegis_llm_presence_penalty`, `aegis_llm_repetition_penalty`, `aegis_llm_thinking_requests_total`, `aegis_llm_thinking_token_count`, `aegis_llm_finish_reason_total`, `aegis_llm_tool_choice_total`. Generation metrics carry low-cardinality `endpoint` and `task_type` labels; chat/async use `task_type="none"`.


### Qwen3.6-27B live rollout contract status (2026-04-24)

- **Active served model / modelId**: `Qwen/Qwen3.6-27B`. Gateway는 `/v1/chat` 요청의 `model` 값을 이 active model로 오버라이드한다.
- **S7 model profile**: `Qwen/Qwen3.6-27B-default` (`/v1/models`, `/v1/health.modelProfiles`에 노출).
- **Serving backend**: DGX Spark `10.126.37.19:8000`, vLLM `0.20.0`, container `qwen36-vllm:hf-fresh`, recipe `qwen3.6-27b-origin`, text-only `--language-model-only`, `--reasoning-parser qwen3`, `--enable-auto-tool-choice --tool-call-parser qwen3_coder`, `--speculative-config {"method":"mtp","num_speculative_tokens":1}`, TP=1, **no quantization override; original dense 27B, not `Qwen/Qwen3.6-27B-FP8`**.
- **Default / fallback**: `Qwen/Qwen3.6-27B`가 S3 `/v1/chat` 및 S7 task path의 품질 우선 기본값이다. 자동 runtime fallback은 없다. `Qwen/Qwen3.6-35B-A3B`는 빠른/simple manual 후보, `Qwen/Qwen3.5-122B-A10B-GPTQ-Int4`는 baseline/archive rollback 후보로만 남긴다.
- **Audit surfaces**: Engine direct `/v1/models`는 `id=root=Qwen/Qwen3.6-27B`, `max_model_len=131072`를 반환한다. Gateway `/v1/models`는 `profileId=Qwen/Qwen3.6-27B-default`, `modelName=Qwen/Qwen3.6-27B`, `contextLimit=131072`를 반환한다. Gateway `/v1/health.modelProfiles`도 동일 profile을 노출한다.
- **Strict JSON / thinking**: `X-AEGIS-Strict-JSON: true`는 계속 지원한다. Gateway가 `response_format={"type":"json_object"}`를 적용하되 caller-supplied `chat_template_kwargs.enable_thinking`은 변경하지 않고, 성공 응답의 `choices[0].message.content`를 parseable top-level JSON object로 검증/compact 정규화하며 `message.reasoning`은 `null`로 scrub한다. `<think>...</think>` 또는 일반 텍스트가 final JSON content에 섞이면 성공으로 반환하지 않고 `502 strict_json_contract_violation` 계열 실패로 처리한다. 따라서 S3 strict finalizer content는 JSON-only로 간주해도 된다.
- **Tool calls**: vLLM은 `qwen3_coder` tool-call parser로 기동한다. Tool-call mode에서는 OpenAI-compatible `message.tool_calls[]`로 분리되며 일반 assistant content에 tool-control text를 섞지 않는다. Final Assessment JSON은 tool-less request + `X-AEGIS-Strict-JSON: true`로 호출해야 하며, 이 경로에서는 tool-control text가 섞이면 strict JSON 실패다. S3의 기존 OpenAI-compatible tool schema/prompt formatting은 변경할 필요가 없다.
- **Effective context / output limits**: live Engine `max_model_len=131072`; Gateway model registry `contextLimit=131072`. `/v1/tasks.constraints.maxTokens`는 caller-owned required field이며 범위는 1..32768이다. `/v1/tasks`의 generation tuple(`enableThinking`, `maxTokens`, `temperature`, `topP`, `topK`, `minP`, `presencePenalty`, `repetitionPenalty`)은 모두 caller가 camelCase `constraints.*` 아래 명시해야 하며 누락 시 422다. `/v1/chat`은 caller의 `max_tokens`, snake_case sampling controls, `chat_template_kwargs.enable_thinking`을 필수로 요구하고 Engine에 전달하되 Gateway가 별도 truncation하지 않는다. Gateway prompt character guard는 `AEGIS_LLM_MAX_INPUT_CHARS=800000`이며, 이는 토큰 한도 보장이 아니라 과대 입력 방어선이다. Long-context extension beyond 131072는 현재 켜지지 않았다. 초과 요청은 Gateway guard 또는 vLLM context/token validation error로 실패하며, S7이 임의로 잘라 성공처럼 반환하지 않는다.
- **Latency / timeout profile**: measured hard benchmark 기준 27B는 qualityScore 0.74/passRate 0.70, mean completion throughput 4.65 tok/s, hard-reasoning p50 latency 약 660.6s / p95 약 1244.8s였다. Lightweight live smoke는 strict JSON 4.975s, tool-call 6.274s, async strict JSON 약 3.8s로 통과했다. S3 권장값은 strict finalizer 600s baseline, large evidence/deep finalizer 900~1800s, tool-call agent turn 600s baseline(긴 추론은 1800s 상한)이다. `/v1/chat` `X-Timeout-Seconds` 기본/상한은 1800s, Gateway connect/write/pool timeout은 10s, task client read timeout은 600s다.
- **Async ownership**: model swap 자체로 status/result/cancel shape는 바뀌지 않았다. 2026-05-08 health-control v2 이후 async ownership은 fixed elapsed read ceiling 없이 wait-while-alive long-running surface로 운영한다. submit/status/result/cancel, active lease refresh, 15분 terminal retention/expiry, strict JSON async failure(`blockedReason=strict_json_contract_violation`, `errorDetail`, `retryable=true`), `/health?requestId=` compact control-signal semantics를 유지한다.


### Caller-owned generation contract (2026-04-29)

S7 Gateway는 더 이상 `temperature=0.3` 또는 `enable_thinking=true` 같은 Gateway-owned generation default를 갖지 않는다. `/v1/chat`, `/v1/async-chat-requests`, `/v1/tasks` caller는 generation controls를 모두 보내야 하며 누락 또는 malformed thinking 값은 422이다.

- `/v1/chat`와 `/v1/async-chat-requests`: OpenAI-compatible snake_case `max_tokens`, `temperature`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `repetition_penalty`, `chat_template_kwargs.enable_thinking` 필수.
- `/v1/tasks`: camelCase `constraints.*` 아래 `enableThinking`, `maxTokens`, `temperature`, `topP`, `topK`, `minP`, `presencePenalty`, `repetitionPenalty` 필수.
- Task path의 thinking 제어는 per-request `constraints.enableThinking`에서 오며, RealLlmClient 생성자 고정값에 의존하지 않는다.
- Gateway 내부 outbound body는 vLLM/OpenAI-compatible snake_case(`max_tokens`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `repetition_penalty`, `chat_template_kwargs.enable_thinking`)로 변환해 전달한다.
...
- Backend is now DGX Spark `10.126.37.19:8000`, vLLM `0.20.0`, image `qwen36-vllm:hf-fresh`, recipe `qwen3.6-27b-origin`, original dense `Qwen/Qwen3.6-27B`, `max_model_len=131072`, MTP speculative decoding `num_speculative_tokens=1`.
- Gateway benchmark runtime for the MTP A/B used `AEGIS_LLM_MODE=real`, `AEGIS_LLM_ENDPOINT=http://10.126.37.19:8000`, `AEGIS_LLM_MODEL=Qwen/Qwen3.6-27B`, `AEGIS_LLM_CONCURRENCY=4`, and `AEGIS_RAG_ENABLED=false` to isolate backend generation/tool-call behavior from RAG retrieval variance. This does not remove the documented RAG integration; it is a benchmark setting.
- Strict bwrap-backed MTP A/B: no-MTP 8/8 aggregate completion throughput `7.638 tok/s`, MTP=1 8/8 `15.132 tok/s` (+98.1%). Tool-call workload improved `4.471 -> 7.492 tok/s` (+67.6%) and latency `9396.5 -> 5606.5 ms` (-40.3%).
- BFCL v4 Gateway smoke: before MTP 25 cases, 24 passed, accuracy `0.96`; after MTP 25 cases, 24 passed, accuracy `0.96`. Only miss was `parallel_multiple_4` expression formatting (`x^2` vs expected `x**2`/lambda), so current evidence shows no MTP tool-call regression.
- Benchmark/report artifacts: `services/llm-gateway/bench/mtp_gateway_ab.py`, `services/llm-gateway/bench/results/mtp-ab-qwen36-27b-strict-20260428T034632Z/s7-qwen36-mtp-benchmark-report.md`, BFCL result directories under `services/llm-gateway/bench/results/bfcl-v4-gateway-smoke-qwen36-27b-*`.


### LLM Engine (DGX Spark)

- **vLLM + Qwen/Qwen3.6-27B**
- `max_model_len: 131072` (live Engine `/v1/models` 기준)
- `RealLlmClient`: task path에서 caller-owned `constraints.enableThinking`을 `chat_template_kwargs.enable_thinking`으로 전달, JSON structured output
- Live smoke: strict JSON 4.975s, tool-call 6.274s, async strict JSON completed
- Hard benchmark: pre-MTP mean completion throughput 4.65 tok/s, p50 660.6s / p95 1244.8s; MTP A/B aggregate 15.132 tok/s vs no-MTP 7.638 tok/s
- 양자화: 없음(bf16/default original dense 27B)
...
`services/llm-gateway/app/generation_policy.py` is the S7-owned Python mirror for Qwen3.6 generation and timeout defaults. It contains the model-pin warning required by the temperature-policy summary: values were validated against `Qwen/Qwen3.6-27B` model-card guidance retrieved on 2026-04-28 and must be revalidated on model family/card change. `/v1/tasks` still remains caller-owned; the constants are for warmup, tests, examples, and downstream caller coordination, not hidden Gateway defaults.
