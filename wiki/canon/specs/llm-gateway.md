...
- **Strict JSON / thinking**: `X-AEGIS-Strict-JSON: true`는 계속 지원한다. Gateway가 `response_format={"type":"json_object"}`와 `chat_template_kwargs.enable_thinking=false`를 강제하고, 성공 응답의 `choices[0].message.content`를 parseable top-level JSON object로 검증/compact 정규화하며 `message.reasoning`은 `null`로 scrub한다. `<think>...</think>` 또는 일반 텍스트가 final JSON content에 섞이면 성공으로 반환하지 않고 `502 strict_json_contract_violation` 계열 실패로 처리한다. 따라서 S3 strict finalizer content는 JSON-only로 간주해도 된다.
...
- `RealLlmClient`: task path에서 thinking 비활성, JSON structured output
