...
- request-aware health tracking: `/v1/tasks`, `/v1/chat`, `/v1/async-chat-requests` 모두 `requestId` 기반 in-flight summary를 노출하며, `llm-inference` 중에는 `localAckState="transport-only"`로 살아있지만 세부 progress-unproven 상태를 표현할 수 있다
- async ownership surface는 durable `requestId`, `traceRequestId`, `resultReady`, `expiresAt`를 가진다
...
- **Async ownership**: model swap으로 async ownership semantics는 바뀌지 않았다. submit/status/result/cancel, 15분 terminal retention/expiry, strict JSON async failure(`blockedReason=strict_json_contract_violation`, `errorDetail`, `retryable=true`), `/health?requestId=` compact control-signal semantics는 유지한다.
