...
| 분석 파이프라인 기반 | AnalysisOrchestrator + AgentClient + SastClient + ProjectSourceService | **구현 완료** |
...
| 테스트 | 단위/통합/계약 테스트 468개 (vitest, 2026-04-20 auth/member-management v1 반영 후 재검증) | **구현 완료** |
...
| 세션 14: 프로젝트 보안 요약 | ProjectListItem + Overview trend + owner 요약 | **구현 완료** |
| 세션 16: 프로젝트 CRUD hardening | blank-name validation + safe delete teardown + blocker-aware `409` 응답 | **구현 완료** |
...
`errorDetail`는 공통 필드 외에 endpoint-specific structured metadata를 추가로 포함할 수 있다. 현재 대표 예시는 project delete conflict의 `errorDetail.blockers` 다.
...
| `FORBIDDEN` | 403 | N | 인증은 되었지만 권한이 부족한 경우 (same-org admin rule 포함) |
| `RATE_LIMITED` | 429 | Y | auth public route rate-limit 초과 |
...
JSON structured logging (stdout, pino). 상세 규약: `wiki/canon/specs/observability.md`
...
- 테이블 (35개): 기존 활성 표면 26개(`projects`, `uploaded_files`, `analysis_results`, `adapters`, `project_settings`, `dynamic_analysis_sessions`, `dynamic_analysis_alerts`, `dynamic_analysis_messages`, `dynamic_test_results`, `audit_log`, `runs`, `findings`, `evidence_refs`, `gate_results`, `approvals`, `build_targets`, `target_libraries`, `sdk_registry`, `notifications`, `users`, `sessions`, `organizations`, `registration_requests`, `password_reset_tokens`, `dev_password_reset_deliveries`, `auth_rate_limit_events`) + execution/persistence seam 9개(`analysis_executions`, `project_source_assets`, `sdk_assets`, `build_units`, `build_unit_revisions`, `build_requests`, `build_attempt_projections`, `build_snapshot_projections`, `build_target_assets`)
- `build_target_assets`는 schema의 legacy table name이며, 현재 의미는 BuildTarget asset mapping 이다.
...
- `analysis_results` 테이블에 `claim_diagnostics TEXT`, `evidence_diagnostics TEXT` 컬럼 추가 — S3 claim/evidence lifecycle diagnostics 보존
- `projects` 테이블에 `owner_id`, `owner_name`, `owner_avatar`, `owner_kind` 컬럼 추가 — 인증된 생성자 프로필 기반 ProjectListItem owner
- `findings` 테이블에 `cwe_id TEXT`, `cve_ids TEXT DEFAULT '[]'`, `confidence_score REAL` 컬럼 추가
- `users` 테이블에 `email`, `organization_id`, `account_status` 컬럼 추가
- 신규 auth/member-management v1 테이블:
  - `organizations` — pre-provisioned org lookup / preview metadata
  - `registration_requests` — org-code member join requests + approval decision + hashed public lookup token
  - `password_reset_tokens` — hashed reset token ledger + expiry/consumption
  - `dev_password_reset_deliveries` — non-production mock bridge outbox carrying latest raw reset tokens for frontend integration only
  - `auth_rate_limit_events` — durable auth public-route / login rate-limit event ledger
- 현재 project delete는 schema-level FK cascade만으로 끝내지 않고, `ProjectDeletionService`의 explicit delete manifest + `uploads/{projectId}` quarantine/restore workflow를 사용한다.

---

## P0: 정적 분석 (연차보고서 필수) ✅ 구현 완료

> **현재 mounted surface note (2026-04-07):**
> 이 섹션은 초기 정적 분석 설계/이력 문맥을 보존한다.
> 현재 S2의 canonical mounted 분석 surface는 `api-endpoints.md`와 `shared-models.md` 기준의 `/api/analysis/*`, `/ws/analysis`, `/api/projects/:pid/pipeline/*`, `/ws/pipeline` 이다.
> 아래 `/api/static-analysis/*` 서술은 historical context로 읽고, 현행 계약 판단에는 사용하지 않는다.

### 현재 mounted 분석 / 진행률 surface 요약
...
WS     /ws/pipeline?projectId=
WS     /ws/upload?uploadId=
WS     /ws/sdk?projectId=
WS     /ws/notifications?projectId=
WS     /ws/dynamic-analysis?sessionId=
WS     /ws/dynamic-test?testId=
```

- **UX intent note (2026-04-13)**: canonical 사용자 여정은 이제 `소스 업로드 + SDK 업로드 → 빌드 준비(Build Agent + compile_commands.json) → Quick 1회성 S4 호출 + S5 GraphRAG 형성 → 사용자의 명시적 Deep 요청` 이다.
- 현재 mounted backend surface는 explicit-step migration 중간 단계다.
- BuildTarget-only cutover 기준 canonical surface:
  - `POST /api/projects/:pid/pipeline/prepare`
  - `POST /api/projects/:pid/pipeline/prepare/:targetId`
  - `POST /api/analysis/quick`
  - `POST /api/analysis/deep`
  - `quick_graphing` analysis WS progress phase
- `POST /api/analysis/run`은 더 이상 mounted되지 않는다.
- `POST /api/analysis/quick`는 `buildTargetId` 기반 Quick 실행이며, `mode`/`targetIds`/project-only payload를 허용하지 않는다.
- `POST /api/analysis/deep`는 `buildTargetId + executionId` 기반 Deep 실행이며, legacy `quickAnalysisId` payload를 허용하지 않는다.
- active analysis progress는 `/ws/analysis`에서 emit되며, current payload는 additive하게 `buildTargetId` / `executionId` traceability를 포함할 수 있다.
...
현재 mounted 분석 실시간 진행률 surface는:

```
WS: /ws/analysis?analysisId=xxx
```
...
```
GET /api/analysis/results?projectId=         프로젝트별 분석 결과 목록
GET /api/analysis/results/:analysisId        개별 분석 결과 조회
```

- 목록: 프로젝트별 분석 결과 반환 (`projectId` 필수 쿼리)
- 개별: SQLite에 저장된 분석 결과 반환 (`deep-${analysisId}` fallback 조회 포함)
- 취약점 목록 포함 (심각도, 출처, 위치, 설명, 수정 가이드)

### P0-6. 대시보드 요약 ✅

```
GET /api/analysis/summary?projectId=<pid>
```

- static + deep 결과를 합산한 대시보드 요약 반환
- `projectId` query 필수, `period` query optional
...
- `requestId` query parameter optional
  - 주어지면 S2가 downstream `/v1/health?requestId=...` 로 그대로 전달한다.
- child service entry는 coarse status와 raw detail을 유지하면서,
  frozen request-aware summary가 있을 때 normalized `control` block을 함께 노출한다.
...
- child service (`llmGateway`, `analysisAgent`, `sastRunner`, `knowledgeBase`, `buildAgent`)
  ...
  - `detail: raw child health payload`
  - `control?: { activeRequestCount, requestId, endpoint, state, localAckState, degraded, degradeReasons, lastAckAt, lastAckSource, blockedReason, pollDecision, decisionReasons }`
...
현재 active + retained WebSocket 엔드포인트:
...
- **`/ws/analysis?analysisId=xxx`** — explicit Quick / explicit Deep 분석 진행률 push
- **`/ws/upload?uploadId=xxx`** — 소스 업로드 진행률 push
- **`/ws/pipeline?projectId=xxx`** — BuildTarget 파이프라인 진행률 push
...
- `url`은 `ws://` 또는 `wss://` 필수
- 프로젝트 삭제 workflow에서 소속 어댑터 disconnect + DB row 정리
...
- exact id 결과를 삭제하고, 가능하면 `deep-${analysisId}` deep 결과도 함께 정리한다.
...
- historical note: 룰 엔진은 제거되었고, 현재 project delete 대상에는 포함되지 않는다.
...
- 프로젝트 삭제 workflow에서 project_settings row도 정리한다.
...
S1↔S2 계약 드리프트를 막기 위해, 현재 mounted backend 의미론을 문서와 계약 테스트로 고정했다. canonical reference는 `wiki/canon/api/shared-models.md`이고, 이 문단은 `backend.md` 관점에서 핵심 mounted 동작만 요약한다.
...
### 사용자 인증 / 회원 관리 v1 ✅ 구현 완료 (lifecycle-first)
...
GET  /api/auth/dev/password-reset/latest?email=...   # non-production bridge
...
### Canonical lifecycle
- org creation / first org-admin bootstrap is **out of scope** for this slice and enters S2 only through seed/migration/import.
- member lifecycle in v1:
  1. org code verify
  2. registration request submitted (`pending_admin_review`)
  3. org-admin assigns role and approves/rejects
...
- login request field keeps the name `username`, but S2 interprets it as a login identifier:
  1. exact legacy `username`
  2. normalized `email`
- token transport remains **opaque session token**; bearer tokens are returned to clients, but S2 stores only a SHA-256 hash at rest in SQLite `sessions`.
- session TTLs:
  - default `24h`
  - remember-me `30d`
- auth rate-limit enforcement is persisted in SQLite `auth_rate_limit_events` for login / org verify / register / password-reset request.
- registration public lookup token TTL `30d`
- password reset token TTL `1h`
- non-production mock bridge defaults:
  - startup seeds fixture organizations/admins when `AEGIS_AUTH_DEV_FIXTURES=true` (or when `NODE_ENV` is `development` / `test`)
  ...
  - `GET /api/auth/dev/password-reset/latest?email=` is enabled when `AEGIS_AUTH_DEV_PASSWORD_RESET_BRIDGE=true` (or when `NODE_ENV` is `development` / `test`) and reads the latest active token from `dev_password_reset_deliveries`

### Admin authorization semantics
- review endpoints require:
  - authenticated session
  - `role = admin`
  - same-organization membership as the target request
- transitional platform-admin bypass exists only for pre-provisioned admins with `organizationId = null`.
- `/api/auth/users` is no longer public. org-admins see only same-org users; platform-admin may list all users.
...
- confirm endpoint consumes the reset token, rotates password, and invalidates all existing sessions for that user.
...
[Express.js Router]
    │
    ├── AnalysisController (BuildTarget-only explicit Quick/Deep surface)
    ...
    │             → LlmTaskClient (v1 Task API 직접 호출)
    │
    ├── ProjectController (CRUD + Overview + safe delete)
    │       → ProjectService
    │             → ProjectDeletionService (blocker / quarantine / restore / DB teardown)
    │
...
├── db.ts                             SQLite 초기화, 테이블 30개 스키마 + FK enforcement + 마이그레이션
├── controllers/
│   ├── health.controller.ts          GET /health (S4,S3,S5,S7,BuildAgent 상태 확인)
│   ├── analysis.controller.ts        legacy `/run` quick alias + explicit Quick/Deep API
...
│   ├── project.controller.ts         CRUD + Overview + safe delete entrypoint
...
├── services/
│   ├── analysis-orchestrator.ts       explicit Quick/Deep 오케스트레이터 + legacy alias 호환
...
│   ├── project-deletion.service.ts    프로젝트 삭제 오케스트레이션
