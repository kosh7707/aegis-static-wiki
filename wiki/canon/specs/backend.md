...
| 테스트 | 단위/통합/계약 테스트 403개 (vitest) | **구현 완료** |
...
- 테이블 (30개): 기존 활성 표면 21개(`projects`, `uploaded_files`, `analysis_results`, `adapters`, `project_settings`, `dynamic_analysis_sessions`, `dynamic_analysis_alerts`, `dynamic_analysis_messages`, `dynamic_test_results`, `audit_log`, `runs`, `findings`, `evidence_refs`, `gate_results`, `approvals`, `build_targets`, `target_libraries`, `sdk_registry`, `notifications`, `users`, `sessions`) + execution/persistence seam 9개(`analysis_executions`, `project_source_assets`, `sdk_assets`, `build_units`, `build_unit_revisions`, `build_requests`, `build_attempt_projections`, `build_snapshot_projections`, `build_target_assets`)
