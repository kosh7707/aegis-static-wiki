...
- 테이블 (29개): 기존 활성 표면 21개(`projects`, `uploaded_files`, `analysis_results`, `adapters`, `project_settings`, `dynamic_analysis_sessions`, `dynamic_analysis_alerts`, `dynamic_analysis_messages`, `dynamic_test_results`, `audit_log`, `runs`, `findings`, `evidence_refs`, `gate_results`, `approvals`, `build_targets`, `target_libraries`, `sdk_registry`, `notifications`, `users`, `sessions`) + snapshot/build persistence seam 8개(`project_source_assets`, `sdk_assets`, `build_units`, `build_unit_revisions`, `build_requests`, `build_attempt_projections`, `build_snapshot_projections`, `subproject_assets`)
...
- `POST /api/analysis/run`은 더 이상 auto Quick→Deep chain을 시작하는 대표 surface가 아니라, `POST /api/analysis/quick`로 수렴하는 **legacy Quick alias** 다.
...
- 이 additive surface들은 explicit-step migration을 위한 중간 단계이며, 아직 최종 public contract cleanup은 끝나지 않았다.
...
[Express.js Router]
    │
    ├── AnalysisController (legacy `/api/analysis/run` quick alias + explicit Quick/Deep surface)
...
├── db.ts                             SQLite 초기화, 테이블 29개 스키마 + FK enforcement + 마이그레이션
├── controllers/
