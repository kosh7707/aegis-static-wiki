...
- `tests/test_static_evidence_consumer_canaries.py` — precomputed full-response JSON consumer canaries proving S3-facing interpretation can be derived from `staticEvidenceContract` only. It emits a versioned exact-key-locked consumer summary (`summarySchemaVersion="s4-static-evidence-contract-consumer-summary-v1"`), commits `benchmark/results/static_evidence/s4-clean-ready-consumer-summary-v1.json` as the canonical clean-ready sample summary, and exposes a pure-JSON smoke CLI (`python -m benchmark.static_evidence_consumer_canary --response <path> [--require-local-static-ready]`) with fixed structured stderr diagnostics for input/output failures. `localStaticEvidenceReady` is a consumer-side completeness verdict: pass/ready/pass gates are necessary but not sufficient; the summary must also project required local coverage surfaces, required claim boundaries/statuses, and current-six tool matrix readiness. It covers top-level and nested contracts, clean ready, failed tool degraded, missing/not-recorded metadata, policy failure, allowed skip, raw `execution.toolResults` poisoning, absent contract, malformed contract behavior, malformed present nested `scan` container classification, and unsafe projection sanitization for statuses/reasons/surfaces/claim IDs/tool IDs/policies. Present non-object map/container projections for `gates`, `coverage`, `claimBoundaries`, `coverage.staticToolExecution`, and individual gate maps are unsafe projection rather than absent-compatible normalization; absent/`None` remains compatible. Duplicate matrix identities are also unsafe projection: duplicate `toolEvidenceMatrix[].toolId` or `claimBoundaryMatrix[].claimId` rows preserve first-observed projected values and mark the summary unsafe so later rows cannot overwrite S3-facing `toolMatrixStatuses`, `toolConsumerPolicies`, `claimSupportStatuses`, or derived `unsupportedClaims`.
...
- `benchmark/static_evidence_consumer_canary.py` is a pure JSON consumer-canary harness. It imports no S4 app modules, ignores raw `execution.toolResults`, and summarizes only `gates`, `coverage`, `claimBoundaries`, `claimBoundaryMatrix`, and `toolEvidenceMatrix` for local-static-readiness testing. The summary projection uses field-specific allowlists for gate statuses, generated reason codes, coverage surfaces, claim IDs, tool IDs, support statuses, and consumer policies; generated partial/degraded/unknown tool evidence remains safe, while malformed/unknown caller-projected values and caller-spoofed summary-only diagnostics (`STATIC_EVIDENCE_CONTRACT_ABSENT`, `STATIC_EVIDENCE_CONTRACT_MALFORMED`, `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION`) are treated as unsafe projection, causing the canary itself to add `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION` to system/evidence/claim reason arrays, force `localStaticEvidenceReady=false`, and are not emitted as tool anomaly reasons.

Verification evidence:

- 2026-05-14: Static Evidence consumer `localStaticEvidenceReady` completeness invariant — the consumer summary now treats positive gates as necessary but not sufficient. Required ready projection includes positive local evidence coverage surfaces (`staticToolExecution`, `sastFindings`, `findingLocations`, `findingCweMapping`, `originClassification`), required `mustNotSupportAlone` boundaries, required claim support statuses (`local-static-artifact=supported`, `absence-of-vulnerability=unsupported`, `cwe-absence=unsupported`), and current-six tool matrix readiness (`ok` with local-tool policy or benign `skipped` with not-applicable policy). If gates say pass/ready/pass but the projected evidence is incomplete, the summary emits canary-generated `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION` only in system/evidence/claim reason arrays and returns not ready; non-ready/degraded reports do not gain that sentinel solely from incomplete projection. RED reproduced forged pass/ready/pass reports with empty/missing projected evidence returning local-ready; targeted completeness tests `4 passed in 0.04s`; static evidence consumer canary tests `38 passed in 0.06s`; static evidence related tests `77 passed in 0.40s`; full S4 pytest `1327 passed in 31.88s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer summary committed artifact and stale-artifact guard — committed `benchmark/results/static_evidence/s4-clean-ready-consumer-summary-v1.json` as the canonical versioned summary for `tests/fixtures/static_evidence_contract/consumer_canaries/clean_ready_top_level.json`. Tests require parsed JSON equality with the helper summary and CLI stdout, and the static evidence results directory rejects extra or drifted summary-schema artifacts while allowing unrelated result JSON. RED reproduced missing committed summary artifact; targeted artifact tests `4 passed in 0.04s`; static evidence consumer canary tests `36 passed in 0.06s`; static evidence related tests `75 passed in 0.40s`; full S4 pytest `1325 passed in 32.47s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer canary malformed nested scan-container classification — `_locate_contract()` now treats present non-object `scan` values as malformed rather than absent while preserving absent/`None` compatibility. This keeps `scan.staticEvidenceContract` non-object behavior malformed, keeps no-contract responses absent, avoids raw container stringification, and gives S3/operator diagnostics the correct `STATIC_EVIDENCE_CONTRACT_MALFORMED` reason. RED reproduced present non-object `scan` returning `contractLocation="missing"`; targeted nested-scan tests `2 passed in 0.04s`; static evidence consumer canary tests `32 passed in 0.06s`; static evidence related tests `71 passed in 0.43s`; full S4 pytest `1321 passed in 32.37s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer canary CLI smoke gate and structured diagnostics — `benchmark/static_evidence_consumer_canary.py` now has a deterministic CLI for response JSON summaries. Successful parses emit the exact helper summary; default exit `0` means a valid present contract summary was emitted, exit `2` means a summary was emitted but the contract is absent/malformed or `--require-local-static-ready` failed, and exit `1` means no summary was emitted due to argument/file/JSON/output failure. Failure stderr uses exact JSON keys `error`, `reasonCode`, `reasonCodes`, `stage`, `summaryEmitted` with fixed reason codes `STATIC_EVIDENCE_CONSUMER_CLI_INPUT_INVALID` and `STATIC_EVIDENCE_CONSUMER_CLI_OUTPUT_FAILED`; raw paths/content/exceptions are not echoed and broken stderr is best-effort. RED reproduced missing CLI entrypoint; targeted CLI tests `7 passed in 0.04s`; static evidence consumer canary tests `30 passed in 0.05s`; static evidence related tests `69 passed in 0.42s`; full S4 pytest `1319 passed in 33.16s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer canary summary schema version and exact-key lock — `benchmark/static_evidence_consumer_canary.py` now emits `summarySchemaVersion="s4-static-evidence-contract-consumer-summary-v1"` on present, absent, and malformed summaries; tests exact-lock the top-level summary key set so accidental additive raw fields or missing projected fields fail fast. This summary schema version is distinct from the underlying `staticEvidenceContract` producer schema. RED reproduced missing version/key-lock evidence; targeted summary-schema test `1 passed in 0.06s`; static evidence consumer canary tests `23 passed in 0.05s`; static evidence related tests `62 passed in 0.42s`; full S4 pytest `1312 passed in 32.51s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer canary duplicate matrix identity fail-closed hardening — `benchmark/static_evidence_consumer_canary.py` now treats duplicate `toolEvidenceMatrix[].toolId` and `claimBoundaryMatrix[].claimId` rows as unsafe multiplicity. First-observed projected values are preserved for tool statuses/policies, claim support statuses, and `unsupportedClaims`; later duplicates are ignored and cannot overwrite S3-facing summary semantics. Unsafe projection adds `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION` only to system/evidence/claim reason arrays, not `toolAnomalyReasonCodes`, and forces `localStaticEvidenceReady=false`. RED reproduced duplicate tool/claim rows leaving local readiness true and unsafe evidence absent; targeted duplicate tests `2 passed in 0.03s`; static evidence consumer canary tests `22 passed in 0.04s`; static evidence related tests `61 passed in 0.39s`; full S4 pytest `1311 passed in 32.15s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer canary map/container-shape fail-closed hardening — `benchmark/static_evidence_consumer_canary.py` now distinguishes absent/`None` object projections from malformed present non-object containers for `gates`, `coverage`, `claimBoundaries`, `coverage.staticToolExecution`, and individual gate maps. Malformed containers are not stringified, normalize to safe defaults, add canary-generated `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION` only to system/evidence/claim reason arrays, never to `toolAnomalyReasonCodes`, and force `localStaticEvidenceReady=false`. RED reproduced malformed `coverage` and `claimBoundaries` preserving local readiness true plus malformed gate containers lacking unsafe reason evidence; targeted container tests `4 passed in 0.04s`; static evidence consumer canary tests `20 passed in 0.04s`; static evidence related tests `59 passed in 0.39s`; full S4 pytest `1309 passed in 32.51s`; Critic implementation+docs review PASS.
- 2026-05-14: Static Evidence consumer canary projection sanitization — `benchmark/static_evidence_consumer_canary.py` now uses field-specific allowlists for S3-facing summaries, preserves generated partial/degraded/unknown tool evidence as safe, and treats malformed statuses/reasons/surfaces/claim IDs/tool IDs/policies plus caller-spoofed summary-only diagnostics (`STATIC_EVIDENCE_CONTRACT_ABSENT`, `STATIC_EVIDENCE_CONTRACT_MALFORMED`, `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION`) as unsafe projection. Unsafe projection adds `STATIC_EVIDENCE_CONTRACT_UNSAFE_PROJECTION` to system/evidence/claim reason arrays, forces `localStaticEvidenceReady=false`, and does not pollute `toolAnomalyReasonCodes`; arbitrary coverage keys are not stringified. RED regressions reproduced `shouldCallS5`/`routeTo` value projection and object-key stringification; unsafe-projection tests `2 passed`, compatibility/fail-closed tests `3 passed`, spoofed-sentinel test `1 passed`, summary-only diagnostic spoofing test `1 passed`, consumer canary tests `16 passed`, static-evidence related tests `51 passed`, full S4 pytest `1293 passed in 32.39s`; Critic implementation+docs review PASS.
