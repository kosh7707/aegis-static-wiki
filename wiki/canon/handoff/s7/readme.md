...
- strict mode에서는 `response_format=json_object`, effective default `enable_thinking=true`, `message.content` JSON object 검증/정규화, caller-facing `reasoning` scrub, 계약 위반 시 502 실패
...
- `/v1/chat` opt-in strict JSON mode 구현:
  ...
  - Gateway가 `response_format={"type":"json_object"}`와 effective default `chat_template_kwargs.enable_thinking=true`를 적용
...
- S3-facing contract is unchanged: strict JSON must return parseable JSON only; Gateway forces `response_format=json_object`, defaults `enable_thinking=true`, strips caller-facing `reasoning`, and fails instead of mixing thinking/tool-control text into final content.
...
## Thinking mode default update (2026-04-28)

- S3 hotN WR 반영: S7 Gateway의 기본 effective thinking은 항상 `chat_template_kwargs.enable_thinking=true`이다. `/v1/chat`, `/v1/async-chat-requests`, `/v1/tasks` real-client path에서 caller가 값을 생략하거나 boolean이 아닌 값을 보내면 Gateway가 `true`를 주입한다.
- caller가 boolean `false`를 명시한 경우에만 mechanical/non-reasoning finalization 요청으로 보존한다.
- strict JSON은 더 이상 thinking-off 예외가 아니다. `response_format=json_object`와 final content JSON object 검증/정규화는 유지하고, successful caller-facing response의 `message.reasoning`은 `null`로 scrub한다. 원본 response와 `effectiveThinking`은 `logs/llm-exchange.jsonl`에 남는다.
- `/v1/chat` 성공 응답에는 `X-AEGIS-Effective-Thinking: true|false` 헤더를 노출한다.
