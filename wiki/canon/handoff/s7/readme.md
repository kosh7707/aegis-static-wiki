...
| 서비스 | 포트/위치 | 역할 |
|--------|-----------|------|
| **LLM Gateway** | :8000 | 5개 taskType + `/v1/chat` 프록시 + opt-in strict JSON mode (LLM 단일 관문) |
...
- strict mode에서는 `response_format=json_object`, `enable_thinking=false`, `message.content` JSON object 검증/정규화, `reasoning` scrub, 계약 위반 시 502 실패
...
| 항목 | 상태 |
|------|------|
| 테스트 | **188 passed** (`.venv/bin/python3 -m pytest -q`, 2026-04-14 검증) |
...
| 미완료 항목 | **live gateway 재기동 후 strict JSON runtime rollout 확인 필요** |
...
- 검증:
  - `tests/test_contract_endpoints.py` **28 passed**
  - 전체 S7 테스트 **188 passed**
  - in-process strict JSON smoke pass
- 운영 주의:
  - 2026-04-14 기준 이미 떠 있는 localhost:8000 프로세스는 재시작 전 코드로 보이며, live smoke에서 strict header 미반영/ reasoning leak가 계속 관측됨
  - 다음 세션은 **runtime restart/rollout 후 live strict JSON smoke 재확인**이 우선
