...
2. `wiki/canon/design-system/DESIGN.md` — §1~§11 전부 (Philosophy / File Layout / Color / Typography / Spacing / Radius-Shadow-Motion / Components / Patterns / Accessibility / Extending / Known gaps)
3. 페이지 구현 중이라면 `wiki/canon/design-system/mocks/{Login,Signup,Dashboard}.html` 중 해당 페이지의 mock을 열어 class/copy/marker를 비교
4. 토큰·컴포넌트 규칙이 불분명하면 `wiki/canon/design-system/assets/{tokens,base,auth-console}.css` + `assets/components/*.css` + `assets/pages/*.css`
...
1. 디자이너 자산에 해당 페이지/컴포넌트가 있는지 확인한다.
...
2. `pages/<Page>/<Page>.tsx` + `pages/<Page>/components/` 구조 유지 (`wiki/canon/feedback/s1_frontend_working_guide.md` §5.3).
3. 페이지 HTML mock이 있으면 그 class·copy·DOM 순서·data-* 속성을 React 구현에 1:1 반영한다.
4. `document.title = "AEGIS — {Page Name}"` 설정, AuthEntryRoute/RequireAuth 중 적절한 가드를 씌운다.
5. `services/frontend/src/styles/handoff/` 와 원본 자산 간 diff를 기록한다. 의도적 adaptation이면 이 문서 §5 동기화 현황에 추가 사유를 남긴다.
