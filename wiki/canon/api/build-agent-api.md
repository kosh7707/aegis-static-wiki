...
Build Agent currently exposes `/v1/tasks` as a synchronous compatibility endpoint to S2. It does **not** currently expose S2-consumable task ownership endpoints such as `GET /v1/tasks/{requestId}`, `GET /v1/tasks/{requestId}/result`, or `DELETE /v1/tasks/{requestId}`.

Current S2 contract:
- `X-Request-Id` correlates logs, health summaries, S4 build ownership, and S7 LLM ownership, but does not by itself create a durable S2-recoverable Build Agent result handle.
...
- If the original `/v1/tasks` HTTP transport is lost before a response body is received, S2 must classify the Build Agent task result as no-result/transport-terminal for that call and may decide whether to submit a new request using its own idempotency policy.
- Build Agent internal S4/S7 calls still follow health-control v2 wait-while-alive semantics; this boundary only describes S2→S3 public result ownership.
...
Build Agent now consumes long-running downstream work through ownership/status/result contracts instead of elapsed-time aborts.

| Surface | Runtime policy |
|---|---|
| S7 LLM async ownership | Submit → status poll → result fetch; queued/running responses stay alive without a fixed async poll deadline. Short HTTP connect/status/result call timeouts remain transport guards. |
| S4 `try_build` | Uses `Prefer: respond-async` against S4 `/v1/build`, derives a child `X-Request-Id` from the parent request id plus endpoint/operation/payload fingerprint, polls `/v1/status/{id}`, then fetches `/v1/result/{id}`. |
| Tool executor | Tools that declare `wait_while_alive=true` are not wrapped in the generic 120s `asyncio.wait_for`; their own ownership loop decides whether downstream is alive, blocked, or failed. |
| Completed envelope | S4 result retrieval and Build Agent envelope assembly must preserve `completed != clean success`; nested build-domain failure remains `cleanPass=false`, not synthetic success. |

Changed `try_build` arguments produce a different S4 child request id because the payload fingerprint changes. An identical retry for the same uncertain build operation reuses the same child id to preserve ownership/idempotency. Build Agent no longer sends an end-to-end `X-Timeout-Ms: 120000` on the durable path; finite HTTP timeouts only protect individual transport calls.
...
```json
{
  ...
  "requestSummary": {
    "requestId": "req-build-123",
    "endpoint": "tasks",
    "state": "running",
    "localAckState": "transport-only",
    "degraded": false,
    "degradeReasons": [],
    "lastAckAt": 1778208893985,
    "lastAckSource": "try_build:s4-ownership-wait",
    "blockedReason": null
  }
}
```
...
- `activeRequestCount` / `requestSummary`는 health-control v2 alive/blocked 판단을 위한 additive field다.
- `requestSummary.localAckState`는 `phase-advancing`, `transport-only`, `ack-break`, 또는 `null`을 사용한다.
...
- S7 async ownership poll은 queued/running 상태를 elapsed wall-clock time만으로 중단하지 않으며, call-level HTTP timeout만 transport guard로 사용
- S4 `try_build`는 durable ownership/status/result 경로를 우선 사용하고, unsupported surface에서만 sync compatibility fallback을 사용
