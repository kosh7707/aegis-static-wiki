...
| `unsafe_output` | `UNSAFE_CONTENT` | 안전하지 않은 출력 때문에 envelope를 completed로 확정할 수 없음 |
| `empty_result` | `EMPTY_RESPONSE` | 빈/무의미한 결과 때문에 envelope를 completed로 확정할 수 없음 |

`COMPILE_FAILED`, `BUILD_SCRIPT_SYNTHESIS_FAILED`, `MISSING_BUILD_MATERIALS`, `SDK_MISMATCH`, `EXPECTED_ARTIFACTS_MISMATCH`처럼 빌드-domain 판단이 가능한 결론은 가능한 한 completed envelope + `cleanPass=false` + diagnostics로 표현한다. S4 build API 호출 예외도 현재 구현에서는 `try_build` 실패 tool result로 먼저 들어오므로, envelope assembly가 가능하면 completed non-clean으로 분류될 수 있다; dependency outage 때문에 envelope assembly 자체가 불가능한 경우에만 non-completed status로 종료한다.
