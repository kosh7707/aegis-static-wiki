...
- Its `localStaticEvidenceReady` boolean is intentionally narrow: `contractPresent && systemStability == "pass" && evidenceReadiness == "ready" && claimSupportReadiness == "pass"`. It is not a security score or vulnerability decision.
...
retained async request에 대해 **best-effort cancellation**을 요청한다. 이 endpoint는 `/v1/scan`, `/v1/build`, `/v1/build-and-analyze` durable ownership mode에 공통 적용된다.
...
- 2026-05-12 로컬 재검증 기준 current six는 모두 alive이며 `policyStatus="ok"`였다. 같은 날 S4 harness fixture의 품질 상태는 별도 report에서 `qualityGate.status="not_decision_grade"`, `qualityGate.localQualityAssessment.status="fail"`이다.
- 따라서 caller는 **시스템 안정성 Gate**와 **품질 Gate**를 분리해야 한다. 도구가 살아 있어도 local oracle 품질이 실패할 수 있고, 품질 실패는 `REQUIRED_TOOL_UNAVAILABLE`/`REQUIRED_TOOL_EXECUTION_INCOMPLETE` 같은 시스템 안정성 오류로 해석하면 안 된다.
