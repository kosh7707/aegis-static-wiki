...
| `state=\"running\"` | 스캔 실행이 시작되었고 local ack가 계속 들어오는 상태 |
| `degraded=true` | 기존 scan heartbeat/execution semantics의 degraded 상태가 health summary에도 반영된 상태 |
| `ackStatus=\"broken\"` + `state=\"failed\"` | local ack break equivalent. 내부 스캔 루프가 예외/정책실패/취소로 비정상 종료되어 upper caller가 abort 대상으로 간주해야 하는 상태 |

### local ack sources

S4가 liveness로 신뢰하는 local ack source는 아래다.

- semaphore acquire (`lastAckSource=\"semaphore-acquired\"`)
...
- `requestSummary.requestId` 가 일치하는 요청에서 `ackStatus=\"broken\"`
