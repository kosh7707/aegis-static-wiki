...
- `deep-analyze`와 `generate-poc` 모두 missing required fields, wrong types, malformed claim objects를 silent normalize하지 않고 strict schema repair를 1회 시도한다. repair 후에도 invalid이면 `validation_failed / INVALID_SCHEMA`로 실패한다.
