...
service_tags: ["s5"]
decision_tags: ["health-control-v2", "timeout-policy", "ack-liveness", "long-running-ownership", "current-state-boundary", "target-context-bundle-v1", "acquisition-envelope-v1", "non-silent-fallback", "knowledge-coverage-contract-v1", "acquisition-readiness-contract-v1", "runtime-vs-offline-evaluation", "analyst-brief-v1", "source-code-kg-ingest-v1", "judge-answer-contract-v1"]
related_pages:
...
| 422 | request schema invalid or graph edge references unknown node |
| 503 | SQLite ledger repository not initialized |
| 408 | caller deadline expires before ledger ingest completes |

## Evidence-Grounded Judge answer contract v1 (internal service contract)

Loop-1 implementation exposes the Judge as an internal S5 module rather than a public HTTP endpoint. Public serving may be promoted in a later Critic-gated loop. The contract exists now so tests/golden sets can validate answerability shape.
...
- `verdict`: `affected | not_affected | unknown | conflicting`
- `status`: `complete | requires_requery | insufficient_input | degraded_quality | stale_cache | policy_blocked`
...
- `forbiddenInferences`: S5 final security verdict, clean pass, S5 accepted claim, exploitability judgment, complete project safety


#### Score Policy v1 additions

Loop 3 adds runtime-configurable scoring policy metadata to Judge answer packets. The Judge remains an internal S5 service contract in this loop, but every answer packet must expose the score vector and the policy evaluation used to interpret it.
...
- `forceContext`: source/target context pins echoed into the canonical query.
- `answerMode`: `evidence_grounded`, `alternatives_without_excluded`, `strict_target_context`, `exploratory_context`.

Control identifiers such as CVE/advisory excludes are whitespace/case normalized before keying and suppression, so `cve-2026-0001`, ` CVE-2026-0001 `, and `CVE-2026-0001` refer to the same exclude. Unsupported known values and unknown control keys are returned under `appliedControls.rejected` and are also visible in `fallbackTrace`; silent fallback is forbidden. Cache keys are decision-fragment keys, not raw prompt text, and include normalized component identity, normalized source context IDs, accepted controls, and answer mode. Score policy is evaluated freshly in the final packet and is not treated as a stale cached final quality gate.
...
Direct Judge conflict population is deferred in this loop. Existing internal Judge packets keep `uncertainty.conflicts` as the placeholder and must not gain S3 final verdict or clean-pass authority. Conflict quality gates are data-quality signals only and do not change `/v1/health`, `/v1/ready`, or projection readiness semantics.
