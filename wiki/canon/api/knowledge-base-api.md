...
> **최종 업데이트**: 2026-05-19 (Judge question credential URL redaction v1 + Judge control credential URL redaction v1 + Judge serving ledger sourceContext request redaction v1 + Judge sourceContext query echo redaction v1 + Source KG partial resolution selector redaction v1 + Judge Source KG validator diagnostic payload budget v1 + Judge Source KG validator diagnostic payload redaction v1 + Judge Source KG nested URL redaction validator v1 + Judge uncertainty field shape validator v1 + Judge fallback trace payload cardinality v1 + Judge uncertainty/follow-up validator v1 + Judge fallback trace payload validator v1 + Judge control effects risk-signal key contract v1 + Judge control effects accepted-control alignment v1 + Judge control effects trace alignment v1 + Judge control effects trace scope v1 + Judge control effects validator v1 + Judge fallback trace validator issue catalog v1 + Judge fallback trace validator v1 + Judge reasoning path validator issue catalog v1 + Judge reasoning path validator case coverage v1 + Judge reasoning path sequence semantics v1 + Judge reasoning path validator v1 + Judge reasoning path policy v1 + Judge fallback trace policy v1 + Judge control effects policy v1 + Judge uncertainty/follow-up policy v1 + Judge verdict policy v1 + Judge answer status policy v1 + Judge quality gate policy v1 + Judge runtime vocabulary policy v1 + Judge forbidden inference policy v1 + Judge relation conflict issue-code catalog v1 + Judge Source KG serving diagnostic catalog v1 + Source KG serving diagnostic coverage v1 + Judge Source KG issue/diagnostic catalog v1 + Judge Threat Retrieval runtime diagnostic coverage v1 + Judge Threat Retrieval runtime diagnostic contract v1 + Judge Threat Retrieval validator issue-code AST guard v1 + Judge Threat Retrieval validator issue-code coverage v1 + Judge Threat Retrieval validator dynamic field catalog v1 + Judge Threat Retrieval validator field catalog v1 + Judge Threat Retrieval validator field normalization v1 + Judge Threat Retrieval validator field path policy v1 + Judge Threat Retrieval validator diagnostic redaction v1 + Judge Threat Retrieval authority issue metadata redaction v1 + Judge Threat Retrieval child authority validator v1 + Judge Threat Retrieval authority boundary validator v1 + Judge Source KG contextResolution issue redaction v1 + Judge Source KG contextResolution shape validator v1 + Judge Source KG contextResolution integrity validator v1 + Judge compile-commands artifact validator tolerance v1 + Judge compile-commands artifact validator v1 + Source KG source artifact resolution accounting v1 + Judge compile-commands artifact contract parity v1 + Judge Source KG URL redaction validator v1 + Source KG compile-commands artifact context v1 + Source KG projection diagnostic budget v1 + Source KG redacted context diagnostic v1 + Judge Source KG nested redaction validator v1 + Source KG serving nested object redaction v1 + Source KG nested object budget v1 + Source KG explicit ID content immutability v1 + Source KG repository root immutability v1 + Source KG line span validation v1 + Ready ledger dependency v1 + Source KG checksum integrity v1 + Analyst Brief selector non-echo v1 + Source KG served URL redaction v1 + Source KG generated identity uniqueness v1 + Source KG producer/reference ID length policy v1 + URL query secret redaction v1 + ETL seed URL redaction v1 + Runtime Qdrant URL log redaction v1 + Judge compact revision hash policy v1 + Judge unsupported control echo budget v1 + Source KG container lineage rebind policy v1 + Judge topK canonicalization policy v1 + Threat Retrieval runtime trace parity v1 + Judge keyword-match fielded policy v1 + Judge forceContext budget policy v1 + Source KG producer ID uniqueness v1 + Source KG atomic ingest policy v1 + Judge missing-version threat context v1 + Judge control list cardinality policy v1 + Source KG producer identity policy v1 + Sync timeout no-side-effect policy v1 + Judge question-term discovery v1 + Judge keyword-match discovery v1 + Source KG ingest size policy v1 + Judge control echo policy v1 + Judge candidate-pool truncation trace v1 + Runtime ledger URL log redaction v1 + Source KG/Judge selector value length policy v1 + Source KG explicit induced-edge cap v1 + Judge semantic expansion response budget v1 + Judge suppressed candidate response budget v1 + Judge Source KG selector limit policy v1 + Judge risk signal response budget v1 + Judge equivalent advisory aggregate response budget v1 + Source KG snippet text truncation policy v1 + Source KG explicit selector limit policy v1 + Source KG transport error reason taxonomy v1 + Source KG rich IR payload truncation policy v1 + Judge equivalent advisory truncation v1 + Source KG runtime error reason taxonomy v1 + Judge missing-input ledger cache trace v1 + Threat Retrieval internal advisory exclude v1 + Source KG missing-container redaction v1 + Source KG out-of-lineage redaction contract v1 + Source KG ingest reference error taxonomy v1 + Source KG out-of-lineage row redaction v1 + Judge conflict-stable cache revision trace v1 + Judge Source KG caller-order preservation v1 + Judge candidate-pool preview trace v1 + Judge relation/conflict revision cache trace v1 + Judge retrieval lab live observation gate v1 + Judge retrieval lab golden cases v1 + exhaustive alias-equivalent fusion v1 + Judge equivalent advisory validator v1 + Judge multi-source advisory fusion v1 + Judge retrieval trace validator v1 + Judge affectedness-first reranker v1 + Judge Source KG degraded status v1 + Judge ledger revision-scoped decision cache v1 + Source KG endpoint error taxonomy split v1 + Source KG explicit ID immutability v1 + Source KG collection lineage validation v1 + Judge conflict value cap validation v1 + Source KG context truncation v1 + Source KG induced-edge context v1 + Source KG selector consistency/order v1 + Source KG context API v1 + Judge relation conflict visibility v1 + Source KG context resolution v1 + Judge Threat Retrieval topK/reranker policy v1 + Judge HTTP serving contract v1 + Knowledge Coverage/Acquisition Readiness Contract v1 + target context acquisition v1 + G008 CVE candidate/discovery split + Analyst Brief v1 + RetrievalPolicy/Quality Lab v1)
