...
service_tags: ["s5", "source-kg-exploration", "coverage-diagnostics"
...
- `POST /v1/paper/finding-context/retrieve` now includes `contextCoverage` so S3 can distinguish overlapping, partial, same-file non-overlapping, unavailable, and unsatisfied Source KG anchor context.
- `POST /v1/paper/source-kg/explore` exposes bounded Source KG exploration (`source_slice`, `function_body`, `callers`, `callees`, `symbol_lookup`, `neighborhood`, `data_flow`), with data-flow returning explicit `not_available` unless selected rich IR/PDG/taint artifacts exist.
...
S5 Source KG coverage/exploration focused: 60 passed in 130.04s; compileall passed
```

> **소유자**: S5 (Knowledge Base)
> **포트**: 8002
> **호출자**: S2 (Backend), S3 (Analysis Agent)
> **최종 업데이트**: 2026-05-21 (S5 Paper Context API는 별도 계약서 `wiki/canon/api/s5-paper-context-api.md`에서 관리; 본 일반 KB API 계약서에는 paper endpoint 분리, no-final-verdict boundary, canonical JSONL/log-analyzer traceability proof, S5_FREEZE_GATE pass, Source KG partial-quality 소비 규칙, coverage diagnostics, exploratory Source KG query surface를 overlay로 반영)
